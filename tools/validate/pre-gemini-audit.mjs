...
assert.match(productionWorkflow, /--max-pages 100/, 'production discovery batch must remain bounded to 100 pages');
assert.match(productionWorkflow, /--max-research-pages 100/, 'production deep-research batch must have its own 100-page bound');
...
const deepResearchEngine = read('tools/discovery/deep-research-engine.mjs');
...
assert.match(discoveryRun, /if \(state\.frontier\.length > 0(?:\s*&&\s*fetcher)?\)/, 'empty web frontier must not be treated as a fatal bootstrap condition');
assert.match(discoveryRun, /runDeepResearch/, 'dedicated deep-research engine must be connected');
assert.match(discoveryRun, /if \(discovery\.state\.researchFrontier\.length > 0 && fetcher\)/, 'deep research must execute directly from researchFrontier');
assert.match(discoveryRun, /maxPages:\s*maxResearchPages/, 'deep research must use its own page budget');
assert.equal(discoveryRun.includes('promoteResearchFrontierForCrawl'), false, 'research URLs must never be promoted into discovery frontier');
assert.equal(discoveryRun.includes('restoreUnprocessedResearchFrontier'), false, 'research URLs must not be restored from discovery frontier');
assert.equal(exists('tools/discovery/research-frontier.mjs'), false, 'obsolete research/discovery queue bridge must remain removed');
assert.match(deepResearchEngine, /state\.researchFrontier/, 'deep-research engine must consume the dedicated research frontier');
assert.match(deepResearchEngine, /state\.researchFrontier = \[\.\.\.finalQueue\.values\(\)\]/, 'deep-research engine must persist only its remaining research queue');
assert.match(deepResearchEngine, /skipReasons/, 'deep-research skip reasons must be observable');
assert.match(deepResearchEngine, /addDiscoveryFrontier/, 'distinct works found during deep research must route back to discovery');
...
assert.match(discoveryRun, /saveDiscoveryState\(statePath, result\.state\)/, 'bounded discovery and research progress must persist');
...
console.log('discovery/research frontier execution: SEPARATE');
console.log('deep-research page budget: INDEPENDENT + BOUNDED');
console.log('deep-research skip reasons: VISIBLE');
